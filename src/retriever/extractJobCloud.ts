...
export class JobExtractorCloud {
  private html: string;
  private dom: JSDOM;
  ...
  async extractJob(url: string): Promise<Partial<Job>> {
    ...
    try {
      ...
      this.fillMissingFieldsFromDOM(job);
  ...
  private extractJsonLd(): JsonLdData | null {
    try {
  ...
  private fillMissingFieldsFromDOM(job: Partial<Job>): void {
    try {
      // Extract title if missing
      if (!job.title) {
        job.title = this.extractTitle();
      }

      // Extract description if missing
      if (!job.description_text) {
        job.description_text = this.extractDescription();
      }
      ...
      const pageContent = this.document.body.textContent || '';
      ...
      if (!job.location?.city && !job.location?.country) {
        job.location = this.extractLocation();
      }
  ...
  private extractLocation(): { city: string; region: string; country: string; remote_policy: string } {
    const selectors = [
  ...
  // ... (rest of the utility methods are the same as in the original extractJob.ts)
